public class FoodBasket_revisionTUE {
    ...
    public static void main(String[] args) {
        ...
        /*getUser().equals(order2.getUser()) User Name: Ajda Pekkan, Age: 75
            ...
            Ezogelin Soup is a soup dish. Order.getUser()
            Details for Order 1:
                    Manti(x 4.0)        Order.getUser();
            Details for Order 1:
                    Manti(x 4.0)
                    TOTAL = 800.0  */
        System.out.println("\n--------Revision Tests--------");
    ...
    public static void surprise(Order order1, Order order2){
        if(order1.getUser().getID() == order2.getUser().getID()){
                order2.setDiscount(order2.getDiscount());
                order2.setPrice(order2.getPrice()*0.8);
                System.out.println("This is a lucky day for " + order1.getUser().getName()+ "! A discount of 20% will be applied to the second order.");
        }
        else{
                System.out.println("OH NO! The orders " + order1.getID() + " and " + order2.getID() + " are for different people." );
        }       
    }
}
